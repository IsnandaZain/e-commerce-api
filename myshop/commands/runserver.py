...
def runserver(port, debug, profiling):
    ...
    if debug:
        app_instance = http.factory(config=MyShopConfig)
    else:
        app_instance = http.factory()

    if profiling:
        app_instance.config["PROFILE"] = True
        app_instance.wsgi_app = ProfilerMiddleware(
